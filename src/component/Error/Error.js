...
export default function Error(props) {
    ...
    useLayoutEffect(() => {
            ...
            .then((res) => {
                ...
                axios.post(host.DEVICE + "/getErrbyUser", { user: props.username, type: "None" }, { secure: true, reconnect: true })
                    .then((res) => {
                        ...
                        list.value = list.value.map((data, index) => ({ ...data, id: index + 1 }))
        ...
        axios.post(host.DEVICE + "/getInfErr", { user: props.username }, { secure: true, reconnect: true })
            .then((res) => {
                ...
                reader.value = reader.value.map((data, index) => ({ ...data, id: index + 1 })) || []
    ...
    const handleUpdate = (e) => {
        ...
        axios.post(host.DEVICE + "/updateErr", { user: props.username, deviceid: deviceid.value, setting: JSON.stringify(register.value) }, { secure: true, reconnect: true }).then(
            (res) => {
                //console.log(res.data)
                if (res.data.status) {
    ...
    const handleAddReader = (e) => {
        ...
        if (newData.length > 0) {
          ...
          axios.post(host.DEVICE + "/addInfErr", { code: err.value, name: "Lỗi " + parseInt(reader.value.length + 1), type: 'Error', infor: JSON.stringify([{ id: 1, text: "..." }]), solution: JSON.stringify([{ id: 1, text: "..." }]), user: props.username }, { secure: true, reconnect: true }).then(
            (res) => {
              ...
              if (res.data.status) {
                reader.value = [
                  ...reader.value,
                  { id: parseInt(reader.value.length + 1), code: err.value, name: "Lỗi " + parseInt(reader.value.length + 1), type: 'Error', infor: [{ id: 1, text: "..." }], solution: [{ id: 1, text: "..." }] },
                ]
                alertDispatch({ type: 'LOAD_CONTENT', payload: { content: dataLang.formatMessage({ id: "alert_5" }), show: 'block' } })
              } else {
                alertDispatch({ type: 'LOAD_CONTENT', payload: { content: dataLang.formatMessage({ id: "alert_3" }), show: 'block' } })
    ...
    return (
        <>
            <div className="DAT_Err">
                <div className="DAT_Err_Banner" style={{ backgroundImage: banner, backgroundPosition: "bottom", backgroundRepeat: "no-repeat", backgroundSize: "cover" }}>
